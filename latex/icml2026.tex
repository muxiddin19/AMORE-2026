...
\begin{enumerate}[nosep,leftmargin=*]
	...
	\item \textbf{State-of-the-art results} on AgentBench (+34.7\% over single-agent), WebArena (+18.6\% over multi-agent baselines), and \mars{}, with comprehensive ablations validating each component.
\end{enumerate}
...
and could be integrated for both efficient communication and adaptive orchestration ( Table \ref{tab:extended_comparison}).
	...
	%\begin{figure*}[t]
%	\centering
%	\input{figures/bar_chart_standalone}
...
%\begin{figure*}[t]
%	\centering
%	\input{figures/communication_efficiency_standalone}
...
\input{figures/comparison_table1}
...
%\label{sec:integration}
 ...
 \subsection{Theoretical Analysis of Adaptive Orchestration}

%\subsubsection{Escalation Convergence}
\paragraph{Escalation as a Markov Decision Process.}
%\textbf{Escalation as a Markov Decision Process}
We analyze the RCM escalation mechanism as a finite-horizon
Markov Decision Process and establish convergence guarantees.

\begin{definition}[Escalation MDP]
	The escalation process is an MDP $\mathcal{M} = (S, A, P, R, \gamma)$ where:
	\begin{itemize}
		\item $S = \Pi \times \{0,...,e_{max}\} \times \{0,...,r_{max}\}$:
		state space (pattern, escalation count, retry count)
		\item $A = \{\text{proceed}, \text{retry}, \text{escalate}, \text{replan}\}$
		\item $P(s'|s,a)$: transition probability based on quality distribution
		\item $R(s,a) = \mathbb{E}[Q(o_t)] - \lambda \cdot \text{cost}(a)$
	\end{itemize}
\end{definition}

\begin{theorem}[Escalation Termination]
	\label{thm:termination}
	For any subtask $t$ with bounded quality variance $\text{Var}[Q(o_t)] \leq \sigma^2$,
	the RCM escalation process terminates in at most
	$T_{max} = (|\Pi| - 1) \cdot e_{max} + r_{max} \cdot |\Pi|$ steps
	with probability 1.
\end{theorem}
\textit{Proof sketch:} The escalation lattice is acyclic by construction (single $\rightarrow$ parallel $\rightarrow$ hierarchical $\rightarrow$ consensus). Each pattern admits at most $r_{max}$ retries before escalation or termination. Since $|\Pi|=4$ and retry/escalation counts are bounded, the process is a finite absorbing Markov chain. Full proof in Appendix~\ref{pr:Escalation Termination}.

\begin{theorem}[Quality Monotonicity]
	\label{thm:monotonicity}
	Under the assumption that $\mathbb{E}[Q(o_t | \pi_i)] \leq
	\mathbb{E}[Q(o_t | \pi_j)]$ for $i < j$ in the escalation order,
	the expected quality is non-decreasing along any escalation trajectory:
	\[
	\mathbb{E}[Q_{final}] \geq \mathbb{E}[Q(o_t | \pi_{initial})]
	\]
\end{theorem}
\textit{When does this hold?} The ordering assumption is empirically validated on MARS: mean quality increases monotonically (single: 0.62, parallel: 0.71, hierarchical: 0.79, consensus: 0.84) for 89\% of subtasks. The 11\% violations occur primarily on highly parallelizable tasks where consensus adds unnecessary coordination overhead. \car{} learns to avoid such cases, routing parallelizable tasks directly to parallel pattern.

\textbf{Cross-Benchmark Validation.} We analyze monotonicity violations on AgentBench and WebArena:
\begin{itemize}[nosep,leftmargin=*]
	\item \textbf{AgentBench:} Monotonicity holds for 84\% of subtasks. Violations concentrate in WebShop (21\% violation rate) where parallel search outperforms hierarchical coordination due to latency sensitivity.
	\item \textbf{WebArena:} Monotonicity holds for 81\% of subtasks. GitLab tasks show 24\% violation rate---parallel issue exploration sometimes yields better coverage than hierarchical delegation.
\end{itemize}

\textbf{Mitigation Strategies.} When escalation fails to improve quality (detected via RCM):
\begin{enumerate}[nosep,leftmargin=*]
	\item \textbf{Rollback:} If $Q(\pi_{j}) < Q(\pi_{i}) - \epsilon_{rollback}$ (with $\epsilon_{rollback}=0.1$), RCM reverts to the previous pattern's output rather than continuing escalation. This occurs in $\sim$7\% of escalations.
	\item \textbf{CAR Learning:} Counter-examples are logged and used in CAR retraining. Over time, CAR learns subtask signatures where escalation is counterproductive, directly routing such tasks to optimal patterns.
	\item \textbf{Pattern Skipping:} For domains with known violations (e.g., WebShop), CAR can skip intermediate patterns in the escalation chain (single $\rightarrow$ consensus directly).
\end{enumerate}
These mitigations reduce effective monotonicity violations to $<$5\% of escalation attempts while preserving the theoretical termination guarantee.

\subsection{Sample Complexity for CAR}

\paragraph{PAC Bounds for Complexity-Aware Routing.}

We establish sample complexity bounds for learning the CAR classifier.

\begin{definition}[Orchestration Regret]
	The orchestration regret for a learned router $\hat{f}_{CAR}$ is:
	\[
	\text{Regret}(\hat{f}_{CAR}) = \mathbb{E}_{t \sim \mathcal{D}}
	\left[ V^*(\pi^*_t) - V^*(\hat{f}_{CAR}(x_t)) \right]
	\]
	where $V^*(\pi)$ is the expected value (success rate minus cost)
	of pattern $\pi$, and $\pi^*_t$ is the optimal pattern for subtask $t$.
\end{definition}

\begin{theorem}[PAC Bound for CAR]
	\label{thm:pac}
	Let $\mathcal{F}$ be the hypothesis class of CAR routers with
	VC dimension $d_{VC}$. For any $\epsilon, \delta > 0$, with
	probability at least $1 - \delta$, a router $\hat{f}_{CAR}$
	trained on $n$ samples satisfies:
	\[
	\text{Regret}(\hat{f}_{CAR}) \leq \epsilon
	\]
	provided that:
	\[
	n \geq \frac{c}{\epsilon^2} \left( d_{VC} \cdot \log\frac{1}{\epsilon}
	+ \log\frac{1}{\delta} \right)
	\]
	where $c$ is a universal constant.
\end{theorem}
\textit{Proof sketch:} The key insight is that orchestration regret decomposes into pattern classification error and cost gap. Unlike model routing (CARROT), our action space has structure: escalation provides recourse, bounding worst-case regret. Applying standard VC theory to the 4-class MLP classifier yields the bound. The orchestration-specific contribution is showing that RCM recourse reduces effective regret by a factor of $(1-p_{\text{escalate}})$, where $p_{\text{escalate}}$ is the escalation success rate ($\sim$0.73 empirically). Full proof in Appendix~\ref{pr:pac}.
 
\begin{corollary}[Label Efficiency]
	For CAR with $W = 3 \times 256^2 \approx 200K$ parameters and
	$\epsilon = 0.1$, $\delta = 0.05$, the required sample size is
	$n \approx O(500)$, consistent with our empirical observation
	that 500 traces achieve 84.5\% accuracy (Table 13).
\end{corollary}
...
%\begin{figure}[t]
%	\centering
%	\input{figures/figure6_cost_performance}
...
Tables \ref{tab:pattern_efficiency} and \ref{tab:mars_results} show results on our \mars{} benchmark. (1) \amore{} achieves \textbf{52.3\%} success, \textbf{18.6\% absolute improvement} over best baseline; (2) \textbf{41\% cost reduction} vs. AgentOrchestra through adaptive pattern selection; (3) Largest gains on Strategic Planning (+8.4\%), which requires most complex orchestration.
...
\begin{figure}[h]
	\centering
	\scalebox{0.6}{\input{figures/figure8_error_analysis}}
...
\begin{figure}[h]
	\centering
	\scalebox{0.6}{\input{figures/figure4_complexity_distribution}}
	\caption{Optimal orchestration pattern by task complexity. Single-agent dominates low-complexity (78\%); consensus necessary for high-complexity (45\%).}
...
\begin{figure}[h]
	\centering
	\scalebox{0.7}{\input{figures/figure6_cost_performance}}
	\caption{Cost-Performance Trade-off on MARS Benchmark. AMORE achieves highest success rate at lower cost than hierarchical alternatives.}
	\label{fig:framework6}
\end{figure}
...
\begin{figure}[h]
	\centering
	\input{figures/communication_efficiency_figure1}
...
\begin{figure}[h]
	\centering
	\scalebox{0.55}{\input{figures/figure7_ablation}}
...
\begin{figure*}[t]
	\centering
	\input{figures/figure1_framework}
...
\begin{figure*}[t]
	\centering
	\input{figures/figure5_agentbench_results}
...
\begin{enumerate}
	\item \textbf{vs. Static Multi-Agent (AutoGen, MetaGPT):} AMORE dynamically selects orchestration patterns rather than using fixed configurations, achieving 18.6\% higher success at 41\% lower cost.
...
\begin{figure*}[t]
	\centering
	\input{figures/figure2_escalation}
...
\begin{figure*}[t]
	\centering
	\input{figures/figure9_scalability}
