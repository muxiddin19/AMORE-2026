...
\textbf{LLM-Based Agents.} Modern agent systems build on ReAct \citep{yao2023react}, with extensions including Chain-of-Thought prompting \citep{wei2022cot}, Tree-of-Thoughts \citep{yao2024tot}, and tool use \citep{schick2023toolformer,qin2024toolbench}. Memory systems like MemGPT \citep{packer2023memgpt} and Larimar \citep{das2024larimar} enable long-term context management.

\textbf{Multi-Agent Systems.} AutoGen \citep{wu2023autogen}, MetaGPT \citep{hong2024metagpt}, and CAMEL \citep{li2023camel} enable collaborative agents. Hierarchical approaches include HALO \citep{chen2025halo} and AgentOrchestra \citep{liu2025agentorchestra}. Communication protocols (A2A \citep{google2025a2a}, MCP \citep{anthropic2024mcp}) standardize inter-agent interaction. LatentMAS \citep{zou2025latent} optimizes communication efficiency through latent-space collaboration.

\textbf{Adaptive Routing.} CARROT \citep{chen2024carrot} provides cost-aware model routing; xRouter \citep{li2024xrouter} and Puppeteer \citep{zhang2024puppeteer} use RL for orchestration; DAAO \citep{chen2024daao} learns difficulty-aware task routing. Unlike these, \amore{} (1) operates at subtask granularity, (2) provides mid-execution recourse via RCM, and (3) maintains cross-agent memory coherence. See Appendix for detailed comparisons.

\textbf{Evaluation.} AgentBench \citep{liu2024agentbench}, WebArena \citep{zhou2024webarena}, and SWE-bench \citep{jimenez2024swebench} evaluate agent capabilities. Our \mars{} benchmark adds orchestration-specific metrics (Table~\ref{tab:orchestration_comparison} in Appendix).
...
\textbf{Metrics.} Beyond task success, \mars{} evaluates orchestration accuracy (optimal patterns selected), adaptation rate (successful adaptations), memory utilization, and cost efficiency.

\textbf{Evaluation Protocol.} Subjective tasks use multi-judge evaluation (2 human experts + 1 calibrated LLM judge, $r > 0.85$ correlation). Pattern labels were annotated by 5 independent domain experts (Cohen's $\kappa = 0.78$). Cross-model judging mitigates circularity. We validate on AgentBench/WebArena to demonstrate gains are not \mars{}-specific. Full protocol details in Appendix~\ref{sub:ann}.
